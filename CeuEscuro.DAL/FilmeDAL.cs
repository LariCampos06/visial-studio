    ...
    public class FilmeDAL : Conexao //Essa classe tem que herdar de conexao pra usar os bang de lá
    {
        ...
        public FilmeDTO SearchMovieById(int movieid)
        {
            try
            {
                ...
                cmd = new MySqlCommand("SELECT * FROM filme WHERE filme.id LIKE @movieid", conn); //A query em si         
                cmd.Parameters.AddWithValue("movieid", movieid);
        ...
        public List<FilmeDTO> Searchbyfilter(string filter)
        {
            try
            {
                ...
                cmd = new MySqlCommand("SELECT filme.Id, filme.Titulo, filme.produtora, filme.URLimg, Classificacao.descricaoclassificacao, genero.descricao FROM filme INNER JOIN genero ON genero.Id = filme.genero_Id INNER JOIN classificacao ON classificacao.Id = filme.Id WHERE genero.descricao = @filter", conn); //A query em si         
                cmd.Parameters.AddWithValue("@filter",filter);
                dr = cmd.ExecuteReader(); 
                
                
                
                //Dr é da classe conexão, utilizamos apenas para ler, diferente da ExecuteNonQuery.
